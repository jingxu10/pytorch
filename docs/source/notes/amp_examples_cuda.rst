...
.. _working-with-unscaled-gradients-cuda:
...
device, like :class:`torch.nn.DataParallel`.  :ref:`The fix is the same<amp-dataparallel-cuda>`:
apply autocast as part of your model's ``forward`` method to ensure it's enabled in side threads.

.. _amp-custom-examples-cuda:
