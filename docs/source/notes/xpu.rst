.. meta::
   ...
   :keywords: memory management, PYTORCH_XPU_ALLOC_CONF, optimize PyTorch, XPU
...
The precision of matmuls can also be set more broadly (limited not just to XPU) via :meth:`~torch.set_float_32_matmul_precision`.
...
.. code:: python
  ...
  a_full = torch.randn(10240, 10240, dtype=torch.double, device='xpu')
  b_full = torch.randn(10240, 10240, dtype=torch.double, device='xpu')
...
.. _XPU 11: https://devblogs.nvidia.com/xpu-11-features-revealed/
...
.. code:: python
  ...
  torch.backends.xpu.matmul.allow_bf16_reduced_precision_reduction = False
...
``XPU_LAUNCH_BLOCKING=1``.  This can be handy when an error occurs on the GPU.
...
call :func:`torch.xpu.synchronize()` before measuring, or use :class:`torch.xpu.Event`
to record times as following::
    ...
    end_event.record()
    torch.xpu.synchronize()  # Wait for the events to be recorded!
...
However, when using non-default streams, it is the user's responsibility to
    ...
    s = torch.xpu.Stream()  # Create a new stream.
    A = torch.empty((100, 100), device=xpu).normal_(0.0, 1.0)
    s.wait_stream(torch.xpu.default_stream(xpu))  # NEW!
    with torch.xpu.stream(s):
        B = torch.sum(A)
...
.. _XPU stream: https://docs.nvidia.com/xpu/xpu-c-programming-guide/index.html#streams
...
``nvidia-smi``. You can use :meth:`~torch.xpu.memory_allocated` and
...
To better understand how XPU memory is being used over time,
:ref:`torch_xpu_memory` describes tools for capturing and visualizing traces of memory use.
...
Optimizing memory usage  with ``PYTORCH_XPU_ALLOC_CONF``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Use of a caching allocator can interfere with memory checking tools such as
``xpu-memcheck``.  To debug memory errors using ``xpu-memcheck``, set
``PYTORCH_NO_XPU_MEMORY_CACHING=1`` in your environment to disable caching.

The behavior of the caching allocator can be controlled via the environment variable
``PYTORCH_XPU_ALLOC_CONF``.
The format is ``PYTORCH_XPU_ALLOC_CONF=<option>:<value>,<option2>:<value2>...``
Available options:
...
* ``backend`` allows selecting the underlying allocator implementation.
  ...
  implementation, and ``xpuMallocAsync``, which uses
  `XPU's built-in asynchronous allocator`_.
  ``xpuMallocAsync`` requires XPU 11.4 or newer. The default is ``native``.
...
* ``max_split_size_mb`` prevents the native allocator
  ...
  can be split. The
  :meth:`~torch.xpu.memory_stats` and
  :meth:`~torch.xpu.memory_summary` methods are useful for tuning.  This
  ...
  With ``backend:xpuMallocAsync``, ``max_split_size_mb`` is ignored.
* ``roundup_power2_divisions`` helps with rounding the requested allocation
  ...
  the native XPUCachingAllocator, the sizes are rounded up in multiple
  ...
  With ``backend:xpuMallocAsync``, ``roundup_power2_divisions`` is ignored.
* ``garbage_collection_threshold`` helps actively reclaiming unused GPU memory to
  ...
  With ``backend:xpuMallocAsync``, ``garbage_collection_threshold`` is ignored.
* ``expandable_segments`` (experimental, default: `False`) If set to `True`, this setting instructs
  the allocator to create XPU allocations that can later be expanded to better handle cases
  where a job changing allocation sizes frequently, such as having a changing batch size.
  Normally for large (>2MB) allocations, the allocator calls xpuMalloc to get allocations
  ...
  `xpuMalloc` a new `(N + 1)*A*B` segment. If there is not enough memory, there is
  ...
  `pinned_use_xpu_host_register` option is a boolean flag that determines whether to
  use the XPU API's xpuHostRegister function for allocating pinned memory instead
  of the default xpuHostAlloc. When set to True, the memory is allocated using regular
  malloc and then pages are mapped to the memory before calling xpuHostRegister.
  This pre-mapping of pages helps reduce the lock time during the execution
  of xpuHostRegister.

  `pinned_num_register_threads` option is only valid when pinned_use_xpu_host_register
...
.. note::
    ...
    :ref:`XPU memory management API<xpu-memory-management-api>`
    are specific to ``backend:native``, and are not meaningful with
    ``backend:xpuMallocAsync``.
    See each function's docstring for details.

.. _XPU's built-in asynchronous allocator:
    https://developer.nvidia.com/blog/using-xpu-stream-ordered-memory-allocator-part-1/

.. _xpu-memory-custom-allocator:

Using custom memory allocators for XPU
---------------------------------------
...
.. code:: C++
   ...
   #include <sys/types.h>
   #include <xpu_runtime_api.h>
   #include <iostream>
   // Compile with g++ alloc.cc -o alloc.so -I/usr/local/xpu/include -shared -fPIC
   extern "C" {
   void* my_malloc(ssize_t size, int device, xpuStream_t stream) {
      void *ptr;
      xpuMalloc(&ptr, size);
   ...
   void my_free(void* ptr, ssize_t size, int device, xpuStream_t stream) {
...
This can be used in python through the :class:`torch.xpu.memory.XPUPluggableAllocator`.
...
.. code:: python
   ...
   # Load the allocator
   new_alloc = torch.xpu.memory.XPUPluggableAllocator(
       'alloc.so', 'my_malloc', 'my_free')
   # Swap the current allocator
   torch.xpu.memory.change_current_allocator(new_alloc)
   # This will allocate memory in the device using the new allocator
   b = torch.zeros(10, device='xpu')
...
For each combination of cuBLAS handle and XPU stream, a cuBLAS workspace will be allocated
...
``torch._C._xpu_clearCublasWorkspaces()`` is called. The workspace size per allocation can be
...
For each XPU device, an LRU cache of cuFFT plans is used to speed up repeatedly
running FFT methods (e.g., :func:`torch.fft.fft`) on XPU tensors of same geometry
...
* ``torch.backends.xpu.cufft_plan_cache.max_size`` gives the capacity of the
  cache (default is 4096 on XPU 10 and newer, and 1023 on older XPU versions).
  Setting this value directly modifies the capacity.

* ``torch.backends.xpu.cufft_plan_cache.size`` gives the number of plans
  currently residing in the cache.

* ``torch.backends.xpu.cufft_plan_cache.clear()`` clears the cache.

To control and query plan caches of a non-default device, you can index the
``torch.backends.xpu.cufft_plan_cache`` object with either a :class:`torch.device`
...
``torch.backends.xpu.cufft_plan_cache[1].max_size = 10``.

.. _xpu-just-in-time-compilation:

Just-in-Time Compilation
------------------------

PyTorch just-in-time compiles some operations, like torch.special.zeta, when
performed on XPU tensors. This compilation can be time consuming
...
.. note::
    ...
    behavior is to call the XPU Runtime API method `xpuGetDeviceCount`_. Because this call in turn initializes the
    XPU Driver API (via `cuInit`_) if it is not already initialized, subsequent forks of a process that has run
    :meth:`~torch.xpu.is_available` will fail with a XPU initialization error.

    One can set ``PYTORCH_NVML_BASED_XPU_CHECK=1`` in your environment before importing PyTorch modules that execute
    :meth:`~torch.xpu.is_available` (or before executing it directly) in order to direct
    :meth:`~torch.xpu.is_available` to attempt an NVML-based assessment (`nvmlDeviceGetCount_v2`_). If the
    NVML-based assessment is successful (i.e. NVML discovery/initialization does not fail),
    :meth:`~torch.xpu.is_available` calls will not poison subsequent forks.

    If NVML discovery/initialization fails, :meth:`~torch.xpu.is_available` will fallback to the standard XPU Runtime
    API assessment and the aforementioned fork constraint will apply.

    Note that the above NVML-based XPU availability assessment provides a weaker guarantee than the default XPU
    Runtime API approach (which requires XPU initialization to succeed). In some circumstances, the NVML-based check
    may succeed while later XPU initialization fails.
...
``XPU_VISIBLE_DEVICES`` environment flag to manage which GPUs are available to
...
.. _xpuGetDeviceCount:
    https://docs.nvidia.com/xpu/xpu-runtime-api/group__XPURT__DEVICE.html#group__XPURT__DEVICE_1g18808e54893cfcaafefeab31a73cc55f

.. _cuInit:
    https://docs.nvidia.com/xpu/xpu-driver-api/group__XPU__INITIALIZE.html#group__XPU__INITIALIZE_1g0a2f1517e1bd8502c7194c3a8c134bc3
...
.. _xpu-graph-semantics:

XPU Graphs
-----------

A XPU graph is a record of the work (mostly kernels and their arguments) that a
XPU stream and its dependent streams perform.
For general principles and details on the underlying XPU API, see
`Getting Started with XPU Graphs`_ and the
`Graphs section`_ of the XPU C Programming Guide.

PyTorch supports the construction of XPU graphs using `stream capture`_, which puts a
XPU stream in *capture mode*. XPU work issued to a capturing stream doesn't actually
...
skips all layers of argument setup and kernel dispatch, including Python, C++, and XPU driver
overheads. Under the hood, a replay submits the entire graph's work to the GPU with
a single call to `xpuGraphLaunch`_.  Kernels in a replay also execute slightly faster
on the GPU, but eliding CPU overhead is the main benefit.

You should try XPU graphs if all or part of your network is graph-safe (usually this means
...
.. _Getting Started with XPU Graphs:
    https://developer.nvidia.com/blog/xpu-graphs/
.. _Graphs section:
    https://docs.nvidia.com/xpu/xpu-c-programming-guide/index.html#xpu-graphs
.. _stream capture:
    https://docs.nvidia.com/xpu/xpu-c-programming-guide/index.html#creating-a-graph-using-stream-capture
.. _xpuGraphLaunch:
    https://docs.nvidia.com/xpu/xpu-runtime-api/group__XPURT__GRAPH.html#group__XPURT__GRAPH_1g1accfe1da0c605a577c22d9751a09597
...
PyTorch exposes graphs via a raw :class:`torch.xpu.XPUGraph` class
and two convenience wrappers,
:class:`torch.xpu.graph` and
:class:`torch.xpu.make_graphed_callables`.

:class:`torch.xpu.graph` is a simple, versatile context manager that
captures XPU work in its context.
...
Example::
    ...
    static_input = torch.empty((5,), device="xpu")
    ...
    static_input.copy_(torch.full((5,), 3, device="xpu"))
    g.replay()
    # static_output holds the results
    print(static_output)  # full of 3 * 2 = 6

    # Fills the graph's input memory with more data to compute on
    static_input.copy_(torch.full((5,), 4, device="xpu"))
    g.replay()
    print(static_output)  # full of 4 * 2 = 8

See
:ref:`Whole-network capture<whole-network-capture>`,
:ref:`Usage with torch.xpu.amp<graphs-with-amp>`, and
:ref:`Usage with multiple streams<multistream-capture>`
for realistic and advanced patterns.

:class:`~torch.xpu.make_graphed_callables` is more sophisticated.
:class:`~torch.xpu.make_graphed_callables` accepts Python functions and
...
:class:`torch.xpu.graph` context and all work in the forward and backward passes
of any callable you pass to :func:`torch.xpu.make_graphed_callables`.
...
* Capture must occur on a non-default stream. (This is only a concern if you use the raw
  :meth:`XPUGraph.capture_begin<torch.xpu.XPUGraph.capture_begin>` and
  :meth:`XPUGraph.capture_end<torch.xpu.XPUGraph.capture_end>` calls.
  :class:`~torch.xpu.graph` and
  :func:`~torch.xpu.make_graphed_callables` set a side stream for you.)
* Ops that synchronize the CPU with the GPU (e.g., ``.item()`` calls) are prohibited.
* XPU RNG operations are permitted, and when using multiple :class:`torch.Generator` instances within a graph,
  they must be registered using :meth:`XPUGraph.register_generator_state<torch.xpu.XPUGraph.register_generator_state>` before graph capture.
  ...
  for managing generator states safely within the graph context. This ensures proper RNG operation and generator management within XPU graphs.
...
* No non-captured XPU work may run in this process (on any thread) while capture is underway.
...
part(s) eagerly and use :func:`torch.xpu.make_graphed_callables` to graph only
the capture-safe part(s).

By default, callables returned by :func:`~torch.xpu.make_graphed_callables`
...
:func:`~torch.xpu.make_graphed_callables` internally creates
:class:`~torch.xpu.XPUGraph` objects, runs warmup iterations, and maintains
static inputs and outputs as needed.  Therefore (unlike with
:class:`torch.xpu.graph`) you don't need to handle those manually.

In the following example, data-dependent dynamic control flow means the
network isn't capturable end-to-end, but
:func:`~torch.xpu.make_graphed_callables`
lets us capture and run graph-safe sections as graphs regardless::

    N, D_in, H, D_out = 640, 4096, 2048, 1024

    module1 = torch.nn.Linear(D_in, H).xpu()
    module2 = torch.nn.Linear(H, D_out).xpu()
    module3 = torch.nn.Linear(H, D_out).xpu()
    ...
    module1 = torch.xpu.make_graphed_callables(module1, (x,))
    module2 = torch.xpu.make_graphed_callables(module2, (h,))
    module3 = torch.xpu.make_graphed_callables(module3, (h,))

    real_inputs = [torch.rand_like(x) for _ in range(10)]
    real_targets = [torch.randn(N, D_out, device="xpu") for _ in range(10)]
...
Usage with torch.xpu.amp
^^^^^^^^^^^^^^^^^^^^^^^^^

For typical optimizers, :meth:`GradScaler.step<torch.xpu.amp.GradScaler.step>` syncs
...
before capture ends::
    ...
    with torch.xpu.graph(g):
        # at context manager entrance, torch.xpu.current_stream()
...
Call :func:`~torch.xpu.make_graphed_callables` on graphable network sections
...
:class:`~torch.xpu.XPUGraph` object and all tensors created during capture
go out of scope.
...
To economize the memory stashed in private pools, :class:`torch.xpu.graph`
and :func:`torch.xpu.make_graphed_callables` optionally allow different
...
:class:`torch.xpu.graph`'s ``pool`` argument is a hint to use a particular private pool,
...
With :func:`torch.xpu.make_graphed_callables`, if you want to graph several
...
:func:`~torch.xpu.make_graphed_callables` will capture their graphs using a shared
private pool.
...
:func:`~torch.xpu.make_graphed_callables` is not allowed. Instead, you must call
:func:`~torch.xpu.make_graphed_callables` separately for each one.
